...
public struct RESTService: Sendable {
    ...
    public func postVoid<Body: Encodable & Sendable>(_ path: String, body: Body? = nil, query: [URLQueryItem] = []) async throws {
        let _: MessageResponse = try await transport.request(path, method: "POST", query: query, body: body)
    }

    public func putVoid<Body: Encodable & Sendable>(_ path: String, body: Body? = nil, query: [URLQueryItem] = []) async throws {
        let _: MessageResponse = try await transport.request(path, method: "PUT", query: query, body: body)
    }
...
extension JSONValue {
    ...
    public var int64Value: Int64? {
        if case let .number(value) = self { return Int64(value) }
        return nil
    }
