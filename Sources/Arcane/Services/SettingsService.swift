...
public struct SettingsService: Sendable {
    ...
    public func getPublicSettings(envID: EnvironmentID? = nil) async throws -> [PublicSetting] {
        ...
        let data = try await rest.transport.rawRequest(path, body: Optional<EmptyBody>.none, authorized: false)
    ...
    public func updateSettings(_ body: UpdateSettings, envID: EnvironmentID? = nil) async throws -> [SettingDto] {
    ...
    public func categories() async throws -> [SettingCategory] {
        let data = try await rest.transport.rawRequest("settings/categories", body: Optional<EmptyBody>.none)
