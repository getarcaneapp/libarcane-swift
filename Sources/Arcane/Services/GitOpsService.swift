...
public struct GitOpsService: Sendable {
    ...
    public func listRepositoriesPaginated(
        ...
        return try await rest.transport.paginated("customize/git-repositories", start: start, limit: limit, query: query)
    }
    ...
    public func testRepository(id: String, branch: String? = nil) async throws {
        ...
        try await rest.postVoid("customize/git-repositories/\(id)/test", body: EmptyBody?.none, query: query)
    }
    ...
    public func browseRepositoryFiles(id: String, branch: String, path: String? = nil) async throws -> GitOpsBrowseResponse {
    ...
    public func listSyncsPaginated(
        ...
        if let sort { query.append(URLQueryItem(name: "sort", value: sort)) }
        if let order { query.append(URLQueryItem(name: "order", value: order.rawValue)) }
        return try await rest.transport.paginated(rest.environmentPath(envID, "gitops-syncs"), start: start, limit: limit, query: query)
    }

    /// Create a new GitOps sync configuration.
    public func createSync(_ body: CreateGitOpsSync, envID: EnvironmentID? = nil) async throws -> GitOpsSync {
    ...
    public func updateSync(id: String, body: UpdateGitOpsSync, envID: EnvironmentID? = nil) async throws -> GitOpsSync {
    ...
    public func performSync(id: String, envID: EnvironmentID? = nil) async throws -> GitOpsSyncResult {
        try await rest.post(rest.environmentPath(envID, "gitops-syncs/\(id)/sync"), body: EmptyBody?.none)
    }

    /// Get the current status of a GitOps sync.
    public func getSyncStatus(id: String, envID: EnvironmentID? = nil) async throws -> GitOpsSyncStatus {
        try await rest.get(rest.environmentPath(envID, "gitops-syncs/\(id)/status"))
    }

    /// Browse files in a synced repository.
    public func browseSyncFiles(id: String, path: String? = nil, envID: EnvironmentID? = nil) async throws -> GitOpsBrowseResponse {
        var query: [URLQueryItem] = []
        if let path { query.append(URLQueryItem(name: "path", value: path)) }
    ...
    public func importSyncs(_ syncs: [ImportGitOpsSyncRequest], envID: EnvironmentID? = nil) async throws -> ImportGitOpsSyncResponse {
