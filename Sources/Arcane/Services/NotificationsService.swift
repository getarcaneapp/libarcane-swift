...
public struct NotificationsService: Sendable {
    ...
    public func getSettings(provider: NotificationProvider, envID: EnvironmentID? = nil) async throws -> NotificationSettings {
        try await rest.get(rest.environmentPath(envID, "notifications/settings/\(provider.rawValue)"))
    }

    /// Create or update notification settings.
    public func upsertSettings(_ body: UpdateNotificationSettings, envID: EnvironmentID? = nil) async throws -> NotificationSettings {
    ...
    public func deleteSettings(provider: NotificationProvider, envID: EnvironmentID? = nil) async throws {
        try await rest.deleteVoid(rest.environmentPath(envID, "notifications/settings/\(provider.rawValue)"))
    }

    /// Send a test notification through the configured provider.
    public func test(provider: NotificationProvider, type: NotificationTestType = .simple, envID: EnvironmentID? = nil) async throws {
    ...
    public func upsertAppriseSettings(_ body: UpdateAppriseSettings, envID: EnvironmentID? = nil) async throws -> AppriseSettings {
        try await rest.post(rest.environmentPath(envID, "notifications/apprise"), body: body)
    }

    /// Send a test notification through Apprise.
    public func testApprise(type: NotificationTestType = .simple, envID: EnvironmentID? = nil) async throws {
